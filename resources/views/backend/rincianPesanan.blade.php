                            ...
                            <thead class="table-dark">
                              ...
                              <tbody>
                                  ...
                                  <tr>
                                    ...
                                    <td>{{ $item->jenis }}</td>
                                    <td>{{ $item->jumlah_beli }}  <strong class="text-info" >{{ $item->satuan}}</strong></td>
                                    <td>{{ \Carbon\Carbon::parse($item->created_at)->isoFormat('dddd D MMMM YYYY') }}</td>
                                    <td class="text-center" >
                                        <form action="/delete-rincian-pesanan/{{ $item->id }}" method="post" class="d-inline" >
                                            @csrf
                                        <input type="hidden" value="{{ $item->barang_id }}" name="barang_id">
                                        <input type="hidden" value="{{ $toko->id }}" name="pesanan_id">
                                        <input type="hidden" value="{{ $item->jumlah_beli }}" name="jumlah_beli">
                                        <button class="btn btn-danger btn-icon  rounded-2" onclick="return confirm('Yakin pesanan {{ $item->nama_barang }} akan dibatalkan.? ') " type="submit"> <i class="mdi mdi-delete text-light"></i> </button>
                                        </form>
                                    </td>
                                  </tr>
                                  @endforeach
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <form action="/pesanan/{{ $idToko }}" method="post">
                ...
                <div class="form-floating mb-3">
                    <select class="form-select  @error('barang') is-invalid  @enderror" id="floatingSelectGrid" aria-label="Floating label select example" name="barang">
                ...
                <div class="form-floating mb-3">
                    <input type="number" class="form-control @error('jumlah_beli') is-invalid  @enderror" id="jumlah_beli" name="jumlah_beli" placeholder="a" value="{{ old('jumlah_beli') }}">
